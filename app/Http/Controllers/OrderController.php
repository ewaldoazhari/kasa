...
//        if ($order->count() > 0) {
//            $order = $order->first();
//            $explode = explode('-', $order->no_struk);
//            $count = $explode[1] + 1;
//            return '-' . $count;
//        }
//        return '-1';
...
//        $customers = Customer::orderBy('name', 'ASC')->get();
        $employees = Employee::role('kasir')->orderBy('name', 'ASC')->get();
...
//        }
        ...
        if (!empty($request->outlet_id)) {
            ...
            $orders = $orders->where('outlet_id','DESC', $request->outlet_id);
...
//    }
    ...
    public function invoicePdf($no_struk)
    {
        $order = Order::where('no_struk', $no_struk)
    ...
    public function invoiceExcel($no_struk)
    {
        return (new OrderInvoice($no_struk))->download('invoice-' . $no_struk . '.xlsx');
    }
}
